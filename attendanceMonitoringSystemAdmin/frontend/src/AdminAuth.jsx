...
// 1. I-define ang templates sa labas ng component para hindi pabalik-balik sa memory
...
export default function AdminAuth() {
  ...
  // 2. Dynamic filter function para sa positions base sa napiling Org
  const getAvailablePositions = () => {
    const selectedOrgName = formData.organization;
    if (!selectedOrgName || selectedOrgName === "Select Org...") return [];

    // Tinitingnan kung may 'OrgType' property galing sa database record
    const foundOrg = organizations.find(
      (org) => org.OrgName === selectedOrgName,
    );
    if (foundOrg && foundOrg.OrgType) {
      return (
        positionTemplates[foundOrg.OrgType] || positionTemplates.defaultClub
      );
    }

    // Fallback mapping gamit ang pangalan
    if (["PADC", "YMO", "CBAM"].includes(selectedOrgName)) {
      return positionTemplates.localGov;
    } else if (selectedOrgName === "SSG") {
      return positionTemplates.studentGov;
    }

    // Automatic 'else' block para sa kahit anong bagong pasok na club sa DB niyo
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin) {
      ...
      if (formData.program === "Select Pro...")
        newErrors.program = "Select Program.";
      if (formData.yearLevel === "Select Yea...")
        newErrors.yearLevel = "Select Year.";
      if (formData.organization === "Select Org...")
        newErrors.organization = "Select Org/Club.";
      if (formData.position === "Select Pos...")
        newErrors.position = "Select Position.";
      if (formData.term_year === "Select Ter...")
        newErrors.term_year = "Select Term/Year.";
  ...
  return (
    <div className="ap-screen-container">
      ...
      <div className={`ap-form-shell ${isLogin ? "" : "ap-layout-flipped"}`}>
        <div
          className={`ap-form-inner ${isAnimating ? "ap-anim-fadeout" : "ap-anim-fadein"}`}
        >
          ...
          <form className="ap-main-form" onSubmit={handleSubmit}>
            ...
            {isLogin ? (
              <div className="ap-stack-vertical">
                <div
                  className={`ap-field-wrapper ${errors.schoolIDNo ? "ap-state-error" : ""}`}
                >
                  ...
                  />
                  {errors.schoolIDNo && (
                    <span className="ap-error-message">
                      {errors.schoolIDNo}
                    </span>
                  )}
                </div>
                <div
                  className={`ap-field-wrapper ap-pos-relative ${errors.password ? "ap-state-error" : ""}`}
                >
              ...
              <div className="ap-stack-register">
                ...
                <div
                  className={`ap-field-wrapper ${errors.email ? "ap-state-error" : ""}`}
                >
                ...
                <div className="ap-row-flexible">
                  <div
                    className={`ap-field-wrapper ${errors.lastName ? "ap-state-error" : ""}`}
                  >
                  ...
                  <div
                    className={`ap-field-wrapper ${errors.firstName ? "ap-state-error" : ""}`}
                  >
                ...
                <div className="ap-row-flexible ap-row-dropdowns">
                  ...
                  />
                  {/* 3. DYNAMIC OPTIONS VALUE CALLED HERE 👇 */}
                  <CustomDropdown
                    label="Position"
                    name="position"
                ...
                <div className="ap-row-flexible ap-pos-relative">
                  ...
                  <div
                    className={`ap-field-wrapper ap-pos-relative ${errors.confirmPassword ? "ap-state-error" : ""}`}
                  >
