...
export default function Settings() {
  ...
  const shouldShowChangePassPopup = focusedField === 'new' && !allChangePassReqsMet;

  const handleInputFocus = (fieldName) => {
    setFocusedField(fieldName);
    if (errors[fieldName]) setErrors(prev => ({ ...prev, [fieldName]: '' }));
    if (passwordErrors[fieldName]) setPasswordErrors(prev => ({ ...prev, [fieldName]: '' }));
  };
  ...
  const runProfileValidation = () => {
    const newErrors = {};
    if (!formData.schoolIDNo.trim()) newErrors.schoolIDNo = 'School ID is required.';
    if (!formData.email.trim()) newErrors.email = 'Email is required.';
    if (!formData.lastName.trim()) newErrors.lastName = 'Last Name required.';
    if (!formData.firstName.trim()) newErrors.firstName = 'First Name required.';
    if (formData.deptID === 'Select Dept' || !formData.deptID) newErrors.deptID = 'Select Dept.';
    if (formData.program === 'Select Program' || !formData.program) newErrors.program = 'Select Program.';
    if (formData.yearLevel === 'Select Year' || !formData.yearLevel) newErrors.yearLevel = 'Select Year.';
      ...
      <div className="settings-grid">
        ...
        <div className="glass-card profile-sidebar-card">
          <div className="avatar-section">
            ...
            {sidebarSuccessMsg && <div className="success-banner" style={{ marginBottom: '1rem', fontSize: '0.85rem', padding: '8px' }}>{sidebarSuccessMsg}</div>}
            
            <div className="avatar-pill-wrapper" style={{ position: 'relative', width: '100px', height: '100px', margin: '0 auto 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#eee', borderRadius: '50%' }}>
              <User size={60} color="#ccc" style={{ margin: 'auto' }} />
              <label className="camera-badge-pill" htmlFor="pfp-upload" style={{ position: 'absolute', bottom: 0, right: 0, background: 'var(--omsc-dark-blue)', color: 'white', padding: '6px', borderRadius: '50%', cursor: 'pointer', display: 'flex' }}>
                <Camera size={16} />
                <input type="file" id="pfp-upload" hidden />
              </label>
            </div>
            <h3 className="sidebar-student-name" style={{ fontSize: '1.2rem', fontWeight: 700 }}>
              {formData.firstName} {formData.lastName}
            </h3>
            <p className="sidebar-student-id" style={{ fontSize: '0.85rem', marginBottom: '1.5rem' }}>
              {formData.schoolIDNo || "No Student ID"}
            </p>
          </div>
          <button type="button" className="btn btn-submit" onClick={handleSidebarSaveClick}>
        ...
        <div className="settings-forms-wrapper">
          ...
          <div className="glass-card settings-form-block">
            <div className="settings-form-header">
              <img src={omscLogo} alt="OMSC Logo" className="settings-college-logo" />
              <div>
                <h3 className="settings-form-heading">Edit Account Information</h3>
                <p className="settings-form-subheading">OMSC Academic Portal Registration Data</p>
              </div>
            </div>
          ...
          <div className="glass-card settings-form-block">
            <h4 className="form-section-title" style={{ marginBottom: '1.5rem', fontWeight: 800, fontSize: '1.3rem' }}>
              Change Security Password
            </h4>

            <form onSubmit={handleChangePasswordSubmit}>
              {passwordSuccessMsg && <div className="success-banner">{passwordSuccessMsg}</div>}

              <div className="registration-stack">
                
                {/* Current Password Field */}
                <div className={`field-group ${passwordErrors.old ? 'has-error' : ''}`}>
                  ...
                  />
                  {passwordErrors.old && <span className="error-text">{passwordErrors.old}</span>}
                </div>
                ...
                <div className="input-row-flex">
                  ...
                  {/* New Password */}
                  <div className={`field-group ${passwordErrors.new ? 'has-error' : ''}`} style={{ position: 'relative' }}>
                    <label className="label-text">New Password</label>
                    <div className={`input-pill-wrapper input-with-icon ${passData.new && !allChangePassReqsMet ? 'error-ring' : ''}`}>
                      <input
                      ...
                      />
                      <button type="button" className="eye-btn-pill eye-btn" onClick={() => setShowNewPass(!showNewPass)}>
                        {showNewPass ? <EyeOff size={18} /> : <Eye size={18} />}
                      </button>
                    </div>
                    
                    {/* Tooltip Requirements */}
                    <div className={`password-popup-tooltip password-requirements-popup ${shouldShowChangePassPopup ? 'visible' : ''}`}>
                      <div className="popup-arrow"></div>
                      <p className="popup-title" style={{ fontWeight: 700, fontSize: '11px', marginBottom: '4px' }}>Requirements:</p>
                      <ul>
                        <li className={changePassReqs.length ? 'met' : 'unmet'}>
                          {changePassReqs.length ? <Check size={12} /> : <X size={12} />} 8+ Characters
                        </li>
                        <li className={changePassReqs.number ? 'met' : 'unmet'}>
                          {changePassReqs.number ? <Check size={12} /> : <X size={12} />} Numbers
                        </li>
                        <li className={changePassReqs.special ? 'met' : 'unmet'}>
                          {changePassReqs.special ? <Check size={12} /> : <X size={12} />} Special Char
                        </li>
                        <li className={changePassReqs.upperLower ? 'met' : 'unmet'}>
                          {changePassReqs.upperLower ? <Check size={12} /> : <X size={12} />} Upper & Lower
                        </li>
                      </ul>
                    </div>
                    {passwordErrors.new && <span className="error-text">{passwordErrors.new}</span>}
                  </div>

                  {/* Confirm Password */}
                  <div className={`field-group ${passwordErrors.confirm ? 'has-error' : ''}`}>
                    <label className="label-text">Confirm New Password</label>
                    <div className={`input-pill-wrapper input-with-icon ${passData.confirm && passData.new !== passData.confirm ? 'error-ring' : ''}`}>
                      <input
                      ...
                      />
                      <button type="button" className="eye-btn-pill eye-btn" onClick={() => setShowConfirmNewPass(!showConfirmNewPass)}>
                        {showConfirmNewPass ? <EyeOff size={18} /> : <Eye size={18} />}
                      </button>
                    </div>
                    {passwordErrors.confirm && <span className="error-text">{passwordErrors.confirm}</span>}
                  </div>
